...
#   {'sampleName' : file}
# sampleXsec1pb should be a dictionary of what each histogram should be scaled to
# {'sampleName' : sampleXsec1pbtion}
...
def analysis(treeVarDict, sample = 'Nominal', Wts = 'Wts', SigWts = 'SigWts', output = True, outFileName = './output.root', jetType = 'CHS', QCDType = 'Pt'):
    ...
    for treeVar in treeVarDict:
        ...
        for key in varHists[treeVar]:
            if 'Data' not in key:
                print key
                varHists[treeVar][key].Scale((sampleXsec1pb[key]/nEvts[key])*lumi)
        if QCDType is 'Pt':
